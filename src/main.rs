...
async fn get_sheet() -> Result<String, Box<dyn std::error::Error>> {
    ...
    for row in table.iter().skip(1) {
        let date = row
            ...
            .as_slice()
            .get(0)
            .map_or_else(|| "Datum missing", |v| v.as_str());
        let sec = row
            .iter()
            .as_slice()
            .get(1)
            .map_or_else(|| "sec missing", |v| v.as_str());
        println!("{} at {} umgeschraubt", date, sec,);

        let section_name = sec.to_string();

        let dtstampdate = date
            .rsplit(".")
            .map(|x| x.to_string())
            .collect::<Vec<String>>()
            .join("");
        ...
        let uid = format!("{}{}", dtstamp, section_name)
            .split_whitespace()
            .collect::<Vec<_>>()
            .join("");
